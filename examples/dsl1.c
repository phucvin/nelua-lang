...
static int64_t examples_dsl1_fib_1(int64_t n);
...
int64_t examples_dsl1_fib_1(int64_t n) {
  int64_t a = 0;
  int64_t b = 1;
  int64_t i = 0;
  while((i < n)) {
...
int nelua_main(int argc, char** argv) {
  nelua_print_1(((nlstring){(uint8_t*)"Hello from Nelua!", 17}));
  {
      nelua_print_1(((nlstring){(uint8_t*)"Hello from DSL!", 15}));
      int64_t n = examples_dsl1_fib_1(10);
      nelua_print_2(n);
  };
  nelua_print_1(((nlstring){(uint8_t*)"Hello from Nelua again!", 23}));
  return 0;
}
